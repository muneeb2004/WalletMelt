...
├── .antigravity/              # Antigravity specialist agents, skills, and checklists
...
* **Read-Fallback Strategy:** `AppState` tries to load category lists and monthly budgets from the Drift repositories. If a Drift read fails (e.g., due to unmigrated tables or database locks), it catches the exception and falls back to the proven sqflite repositories.
...
* **Purpose:** Defines the domain types used throughout the UI and controllers, decoupling code from raw SQL row representations.
* **Structure:**
...
1. **Dashboard (`/`)**:
   - Spacing, summary cards, current budgets, quick insights, and a list of recent expenses.
2. **History (`/history`)**:
...
* **Typography:** Integrates systemic sizes utilizing the Outfit font family.
* **Tokens:** Colors, border radii, text configurations, and spacing guidelines are stored as standard `ThemeData` properties to ensure consistent widgets.
