...
class DriftExpenseRepository {
  ...
  Future<List<domain.Expense>> listActive() async {
    final rows = await (_db.select(_db.expenses)
          ...
          ..orderBy([
            (expense) => OrderingTerm(expression: expense.date, mode: OrderingMode.desc),
            (expense) => OrderingTerm(expression: expense.createdAt, mode: OrderingMode.desc),
          ]))
        .get();
  ...
  Future<List<domain.Expense>> listDeleted() async {
    final rows = await (_db.select(_db.expenses)
          ...
          ..orderBy([
            (expense) => OrderingTerm(expression: expense.deletedAt, mode: OrderingMode.desc),
          ]))
        .get();
    return rows.map(_toDomain).toList();
  }

  Future<domain.Expense?> getById(String id, {bool includeDeleted = false}) async {
    final query = _db.select(_db.expenses)..where((expense) => expense.id.equals(id));
    if (!includeDeleted) {
  ...
  Future<domain.Expense> create(ExpenseDraft draft) async {
    ...
    await _db.transaction(() async {
      ...
      await _replaceGroceryItems(expense.id, draft.groceryItems, now, expense.currency, expense.categoryId);
      if (expense.receiptImageUri != null) {
        await _insertReceiptForLegacyPath(expense.id, expense.receiptImageUri!, now, null);
      }
    });
    return expense;
  }

  Future<void> update(domain.Expense expense, {List<GroceryItemDraft>? groceryItems}) async {
    final updatedAt = DateTime.now().toIso8601String();
    final updated = expense.copyWith(updatedAt: updatedAt);
    await _db.transaction(() async {
      await (_db.update(_db.expenses)..where((row) => row.id.equals(expense.id))).write(_expenseUpdateCompanion(updated));
      if (groceryItems != null) {
        await _replaceGroceryItems(expense.id, groceryItems, updatedAt, expense.currency, expense.categoryId);
      }
      await (_db.delete(_db.receipts)..where((row) => row.id.equals(_legacyReceiptId(expense.id)))).go();
      if (updated.receiptImageUri != null) {
        await _insertReceiptForLegacyPath(updated.id, updated.receiptImageUri!, updatedAt, updated.deletedAt);
      }
    });
  }
  ...
  Future<void> softDelete(String id) async {
    final now = DateTime.now().toIso8601String();
    await (_db.update(_db.expenses)..where((expense) => expense.id.equals(id))).write(
  ...
  Future<void> permanentlyDelete(String id) async {
    await (_db.delete(_db.expenses)..where((expense) => expense.id.equals(id))).go();
  }

  Future<List<domain.GroceryItem>> groceryItemsForExpense(String expenseId) async {
  ...
  Future<void> _replaceGroceryItems(
    ...
    await (_db.delete(_db.groceryItems)..where((item) => item.expenseId.equals(expenseId))).go();
    await (_db.delete(_db.expenseItems)..where((item) => item.expenseId.equals(expenseId))).go();

    for (final item in items.where((item) => item.name.trim().isNotEmpty && item.amount > 0)) {
      final id = _uuid.v4();
      final name = item.name.trim();
      final normalizedName = _normalizeName(name);
      final canonicalItem = await _findOrCreateItem(name: name, normalizedName: normalizedName, categoryId: categoryId, now: now);
  ...
  Future<local.Item> _findOrCreateItem({
    ...
    final existing = await (_db.select(_db.items)..where((item) => item.normalizedName.equals(normalizedName))).getSingleOrNull();
    ...
    return (await (_db.select(_db.items)..where((item) => item.id.equals(itemId))).getSingle());
  }

  Future<void> _insertReceiptForLegacyPath(String expenseId, String uri, String createdAt, String? deletedAt) async {
